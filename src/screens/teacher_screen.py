...
def teacher_dashboard():
    ...
    with c1:
        header_dashboard()
    with c2:
        st.markdown(f"<h3 style='text-align: right; margin-bottom: 10px;'>Welcome, {teacher_data['name']}</h3>", unsafe_allow_html=True)
        
        _, btn_col = st.columns([1, 1.2]) 
        with btn_col:
            if st.button("Logout ⌘ + Backspace", type='secondary', key='loginbackbtn', use_container_width=True):
...
def teacher_tab_take_attendance():
    ...
    with col2:
        if st.button('Add Photos',type='primary',icon=':material/imagesmode:',width='stretch'):
            add_photos_dialog()
    ...
    if st.session_state.attendance_images:
        ...
        for idx, img in enumerate(st.session_state.attendance_images):
            with gallery_cols[idx % 4]:
                st.image(img, use_column_width='always', caption=f'Photo {idx+1}')

    # FIX 1: Use parentheses instead of square brackets
        ...
        if st.button('Run Face Analysis', width='stretch', type='secondary', icon=':material/analytics:', disabled=not has_photos):
            with st.spinner('Deep scanning classroom photos...'):
                ...
                for idx, img in enumerate(st.session_state.attendance_images):
                    ...
                    # FIX 2: Indented this block so it runs for EVERY image, not just the last one
                    if detected:
...
def teacher_tab_manage_subjects():
    ...
    col1, col2 = st.columns([1.5, 1], vertical_alignment='bottom') 
    with col1:
        st.markdown("<h1 style='font-size: 3.5rem; line-height: 0.9; margin-bottom: 10px;'>Manage<br>Subjects</h1>", unsafe_allow_html=True)
   
    with col2:
        if st.button('Create New Subject', type='secondary', use_container_width=True):
...
def teacher_screen_login():
    ...
    teacher_username = st.text_input("Enter username", placeholder='rajwardhan')
...
def teacher_screen_register():
    ...
    teacher_username = st.text_input("Enter username", placeholder='rajwardhan')
    teacher_name = st.text_input("Enter name", placeholder='rajwardhan')
    teacher_pass = st.text_input("Enter password", type='password', placeholder='enter password')
