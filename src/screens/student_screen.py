...
def student_dashboard():
    ...
    for i,sub_node in enumerate(subjects):
        ...
        def unenroll_btn(current_sid=sid):
            if st.button("Unenroll from this course", type='tertiary', key=f"unenroll_{current_sid}", use_container_width=True):
...
def student_screen():
    ...
    if photo_source:
        ...
        # Placed the registration form correctly inside the photo_source block
        if show_registration:
            # Fixed missing colon
            with st.container(border=True): 
                st.header('Register new Profile')
                new_name = st.text_input("Enter your name ", placeholder='E.g raj patil')
                ...
                if st.button('Create Account', type='primary'):
                    if new_name:
                        with st.spinner('Creating profile..'):
                            # Removed the duplicate Image.open() here. We reuse 'img' from above.
